...
class ShoeMetrics extends Component {
  ...
  getShoes = () => {
    shoeAPI.getShoesByUserId(this.props.userId).then(res => {
      this.setState({
        shoes: res.data
      });
    });
  ...
  addShoe = () => {
    ...
    shoeAPI.addShoe(shoe).then(res => {
      this.getShoes();
    });
  };

  deleteShoe = id => {
    shoeAPI.deleteShoe(id, this.props.userId).then(res => {
      this.getShoes();
    });
  };
  ...
  render() {
    return (
      <span>
        <button
          className="btn btn-outline-dark btn-sm shoesBtn"
          onClick={this.openShoesModal}
        >
          My Shoes
        </button>

        <Modal open={this.state.openShoesModal} onClose={this.closeShoesModal}>
          <h4>Your Shoes</h4>
          ...
          <div className="shoeModal">
            <div
              className={`input-group input-group-sm shoeInputForm saved-${this.state.saved}`}
            >
              {/* SHOE NAME */}
              <input
                autoComplete="off"
